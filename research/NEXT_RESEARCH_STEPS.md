...
both at construction/product-distance level and, separately, for induced
\(K\). Its
...
\]
For the same unchanged core, (KPGE2-1)--(KPGE2-45) prove the sole maximizer
\(\{4m+1,\ldots,10m+2\}\) and
\[
K={1714m^3+1353m^2+24mq+281m+12q^2+36q+30\over6}.
\]
The five regular residue branches have coefficient \(857/3000\), with
\(m=1\) the sole residual of that branch expansion; the all-row formula
remains exact. Every deletion gain and shortcut is
strict; the shortcut minimum lies on the retained closing label for
\(m=1,2,3\) and on \(c_0\) thereafter. Exact subtraction proves a strict
...
\]
No finite rounding or five-prefix conclusion is part of that optimization. A
...
(u+v-n-r_n)^2
\]
and proves the certified residual bound
\[
\gamma^{(r_n^*)}_{1,n}-P^*_{r_n^*,n}
\ge
{26-15\sqrt3\over54}n^3-O(n^2).
\]
...
\]
and complete exact boundary analysis makes
\((\alpha_*,\beta_*,\lambda_*)\) the unique global maximin point. Intact
base-edge splits
...
the resulting scalar lower bounds. Exact rounding proves, for \(n\ge90\),
\[
...
\]
On \([1/3,1]\) its unique maximum is the degenerate endpoint value \(1/3\)
...
- EXACT THEOREM (JOINT ONE-PREFIX LINEAR BLOCK, GLOBAL CONSEQUENCE): for
  ...
  \(s_n^*=\lceil\beta_*n\rceil\), the literal floor/ceiling domain is
  \(n\ge86\), and
  \[
  \Lambda_n
  \ge\Gamma_n^{(r_n^*)}
...
- EXACT THEOREM (NORMALIZED \(k\)-PREFIX SIMPLEX): for every \(k\ge1\),
  ...
  \qquad x_0=1,
  \]
  has one strictly interior maximizer. With \(M_0=0\),
  \[
...
- VERIFIED FACT (FINITE EXACT DOSSIER DIAGNOSTIC): an independent
  ...
  equation, strict feasibility, and monotonicity. A separate denominator-12
  rational-grid oracle compares literal enumeration with an independent
  discrete Bellman calculation and finds no value above \(M_k\).
...
13. For the joint one-prefix family
    ...
    C(\alpha,\beta,\lambda)
    =p(\alpha)+(\alpha-\beta)
      {\lambda\{4(1+\alpha)\beta-(1+\alpha)^2
                       -2\lambda\beta^2\}
       \over2(2-\lambda)}.
    \]
    ...
    prefixes remain literal. The proof-valid positive region is
    \(\alpha>1/3\), \((1+\alpha)/4<\beta<\alpha\), and
    \(0<\lambda<\{4(1+\alpha)\beta-(1+\alpha)^2\}/(2\beta^2)\), with
    \(\lambda\le1\). Exact elimination, including every boundary face, gives
...
- CLOSED METHOD QUESTION: the general consecutive-tail refinement has an
  ...
  \(R_2^*(n)\).
- CLOSED METHOD QUESTION (JOINT FIRST LINEAR BLOCK): for \(m=1\), the exact
  base-slack and selected-prefix proof is now optimized jointly over
  \(r_n=\lfloor\alpha n\rfloor\), \(s_n=\lceil\beta n\rceil\), and the
  prefix weight \(\lambda\). The unique template maximizer and every boundary
  face are classified, and the resulting global and geometric lower
  coefficients are displayed above. The residual and global constants remain
  certified lower coefficients, not exact asymptotic values or convergence
  results.
...
- Generalized the first linear-block certificate to
  \(r_n=\lfloor\alpha n\rfloor\) and jointly optimized
  \((\alpha,\beta,\lambda)\). Exact algebra gives the proof-valid and
  positive-cubic regions, all floor/ceil terms, complete boundary analysis,
  and the unique template maximizer
  \((1-\sqrt3/3,\ 5/6-\sqrt3/4,\ (88-32\sqrt3)/73)\). The resulting global
  lower coefficient is \((4+2\sqrt3)/27\), with an exact finite theorem from
  \(n=86\) and a simpler polynomial theorem from \(n=90\). Bounded exact
  diagnostics cover all 6,972 depth-two histories at \(n=141\), including
...
- Extended the direct one-use charging proof exactly to five selected
  ...
  segments, the original-edge slack has one canonical charged/unused
  partition, and the recursive endpoint invariant covers every child through
  all four boundaries. One standalone exact oracle checks every local
  five-split history of a five-edge base, including 120 all-five-base histories
  and 2,520 fifth splits between two previously inserted labels. No
...
- The even-\(v\) residue-two PG49-star construction, \(W\), and separate
  induced-\(K\) tasks are complete. On \(n=10m+2\), \(m\ge1\), the single
  ...
  \((8m+4)(8m+2)/2\). Its unchanged core has sole induced maximizer
  \(\{4m+1,\ldots,10m+2\}\), exact score (KPGE2-4), coefficient
  \(857/3000\), and a strict all-row improvement over both the known
  residue-two order and K825. Nine hole classes, every shortcut length,
  doubleton, singleton block, cyclic cut, all empty ranges, and \(m=1,2\)
