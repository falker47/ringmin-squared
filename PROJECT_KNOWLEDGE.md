...
- VERIFIED FACT: `build_bounded_small_n_interval_certificate_fixture` builds a finite small-n interval certificate only when the regenerated canonical order count is at most the caller's explicit `max_canonical_orders` bound.
- VERIFIED FACT: `build_n4_interval_certificate_fixture`, `export_n4_interval_certificate_artifact`, and the `power-ringmin-export-n4-interval-certificate` CLI build, validate, write, and reload the bounded finite n=4 interval certificate artifact using `max_canonical_orders=3` by default.
- VERIFIED FACT: `examples/small_n_interval_certificate_n4.json` is a checked finite n=4 interval certificate artifact generated by `power-ringmin-export-n4-interval-certificate --output examples/small_n_interval_certificate_n4.json --digits 80 --guard-decimal 1e-70 --radius-eta 1e-4 --max-canonical-orders 3 --local-max-attempts 8 --created-at-utc 2026-07-11T00:00:00Z`.
- COMPUTER-CERTIFIED RESULT: under the repository's documented local interval-verifier semantics and guarded `mpmath.iv` interval backend contract, `examples/small_n_interval_certificate_n4.json` verifies one local fixed-order interval bracket for each of the three canonical n=4 cyclic orders and records the finite global bracket `(1.4955284118749971877804227915476076304912567138671875, 1.4957284118749971657535979829845018684864044189453125]`.
- INTERPRETATION: the n=4 global interval certificate artifact is finite n=4 evidence only; it is not an exact optimum value, not a theorem for all `n`, not an asymptotic result, and still carries the interval-backend provenance limitation documented for the local verifier/exporter.
- VERIFIED FACT: the interval-certificate production hardening task changed the local interval verifier to parse certificate decimal strings at the interval oracle's recorded precision, rather than at the ambient global `mpmath` precision.
...
- VERIFIED FACT: the small-n interval certificate validator now checks the full problem/order-space/aggregation metadata contract, provenance essentials, evidence check contract, and recomputed local summary diagnostics in addition to re-verifying every embedded local fixed-order interval bracket.
- VERIFIED FACT: `power-ringmin-export-small-n-interval-certificate` is a bounded general finite small-n certificate CLI that requires an explicit `--max-canonical-orders` ceiling and supports `--dry-run` preflight without writing an artifact.
- VERIFIED FACT: under the current canonical cyclic order convention, `n=5` has 12 canonical orders; the hardening task verified this through the bounded CLI dry-run test before any `n=5` certificate artifact was checked in.
- VERIFIED FACT: the 2026-07-12 bounded `n=5` certificate attempt ran `power-ringmin-export-small-n-interval-certificate` through the source tree with `--n 5 --max-canonical-orders 12` and generated `ops/TASK-20260712__bounded_n5_interval_certificate_attempt/small_n_interval_certificate_n5_attempt.json`.
- COMPUTER-CERTIFIED RESULT: under the repository's documented local interval-verifier semantics and guarded `mpmath.iv` interval backend contract, the 2026-07-12 `n=5` task artifact verifies one local fixed-order interval bracket for each of the 12 canonical n=5 cyclic orders and records the finite global bracket `(3.934227717145796443531935437931679189205169677734375, 3.9344277171457964215051106293685734272003173828125]`.
- INTERPRETATION: the 2026-07-12 `n=5` task artifact is finite n=5 evidence only; it is not an exact optimum value, not a theorem for all `n`, not an asymptotic result, and its artifact provenance records `git_dirty=true` because task dossier files existed during generation.
- VERIFIED FACT: `examples/small_n_interval_certificate_n5.json` is a checked finite n=5 interval certificate artifact generated from a clean worktree by `power-ringmin-export-small-n-interval-certificate --n 5 --max-canonical-orders 12 --output examples/small_n_interval_certificate_n5.json --digits 80 --guard-decimal 1e-70 --radius-eta 1e-4 --local-max-attempts 8 --created-at-utc 2026-07-12T00:00:00Z`.
- VERIFIED FACT: `examples/small_n_interval_certificate_n5.json` records source commit `1b013793eafe03661d12e0c1ae5aec3e9173d151` and `git_dirty=false` in its repository provenance.
- COMPUTER-CERTIFIED RESULT: under the repository's documented local interval-verifier semantics and guarded `mpmath.iv` interval backend contract, `examples/small_n_interval_certificate_n5.json` verifies one local fixed-order interval bracket for each of the 12 canonical n=5 cyclic orders and records the finite global bracket `(3.934227717145796443531935437931679189205169677734375, 3.9344277171457964215051106293685734272003173828125]`.
...
- VERIFIED FACT: `examples/small_n_interval_certificate_n6.json` is a checked finite n=6 interval certificate artifact regenerated from a clean worktree by `power-ringmin-export-small-n-interval-certificate --n 6 --max-canonical-orders 60 --output examples/small_n_interval_certificate_n6.json --digits 80 --guard-decimal 1e-70 --radius-eta 1e-4 --local-max-attempts 8 --created-at-utc 2026-07-12T00:00:00Z`.
- VERIFIED FACT: `examples/small_n_interval_certificate_n6.json` records source commit `32743f7f791c8a1550689e39f56347a194d4520a` and `git_dirty=false` in its repository provenance.
- COMPUTER-CERTIFIED RESULT: under the repository's documented local interval-verifier semantics and guarded `mpmath.iv` interval backend contract, `examples/small_n_interval_certificate_n6.json` verifies one local fixed-order interval bracket for each of the 60 canonical n=6 cyclic orders and records the finite global bracket `(8.4678350760883720482752323732711374759674072265625, 8.4680350760883715821591977146454155445098876953125]`.
- INTERPRETATION: the checked n=6 global interval certificate artifact is finite n=6 evidence only; it is not an exact optimum value, not a theorem for all `n`, not an asymptotic result, and still carries the interval-backend provenance limitation documented for the local verifier/exporter.
...
- VERIFIED FACT: `pyproject.toml` registers the console script `power-ringmin-export-n4-interval-certificate`.
- VERIFIED FACT: `pyproject.toml` registers the console script `power-ringmin-export-small-n-interval-certificate`.
...
- VERIFIED FACT: `python -m pytest tests\test_small_n_interval_certificate.py` passed 12 finite small-n interval certificate tests after adding the bounded n=4 artifact/export path on 2026-07-11.
- VERIFIED FACT: `python -m pytest` passed 60 tests after adding the bounded n=4 interval certificate artifact/export path on 2026-07-11.
- VERIFIED FACT: `python -m pytest tests\test_interval_verifier.py tests\test_small_n_interval_certificate.py` passed 24 interval verifier/certificate tests after interval-certificate production hardening on 2026-07-11.
- VERIFIED FACT: `python -m pytest` passed 66 tests after interval-certificate production hardening on 2026-07-11.
- VERIFIED FACT: `python -m pytest tests\test_interval_verifier.py tests\test_small_n_interval_certificate.py` passed 24 focused interval verifier/certificate tests before the bounded n=5 certificate attempt on 2026-07-12.
- VERIFIED FACT: `python -m pytest` passed 66 tests after the bounded n=5 certificate attempt on 2026-07-12.
...
- VERIFIED FACT: `python -m pytest tests\test_small_n_interval_certificate.py` passed 19 focused small-n interval certificate tests after promoting the checked n=6 example artifact on 2026-07-12.
- VERIFIED FACT: `python -m pytest` passed 68 tests after promoting the checked n=6 example artifact on 2026-07-12.
...
- VERIFIED FACT: no promoted `examples/` finite global quadratic-radii certificate artifact beyond the checked finite n=6 example has yet been established in this repository.
