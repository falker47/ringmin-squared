"""Independent bounded checks for the monotone threshold-closing PG46 shift.

This standalone script imports only the Python standard library. It builds
the one prescribed interval shift directly, expands the retained scaffold,
scores induced cycles with an increasing-path max-plus dynamic program, and
checks every oriented compressed shortcut on bounded rows. It enumerates no
subset, path permutation, matching, or family of cyclic orders.
...
def monotone_assignment(m: int) -> tuple[int, ...]:
    """Return alpha_{q,2m-1} directly, without any search."""
    assert m >= 3
    q = (4 * m + 3) // 5
...
def check_ferrers(m: int, alpha: tuple[int, ...]) -> None:
    """Check bijectivity, increasing residual paths, and every Ferrers edge."""
    v = 2 * m
    q = (4 * m + 3) // 5
    kappa = thresholds(m)
    assert len(alpha) == v
...
def score_formula(m: int) -> int:
    """Return the exact q-dependent formula for the prescribed order."""
    q = (4 * m + 3) // 5
    numerator = 572 * m**3 + 619 * m**2 + 8 * m * q + 207 * m + 4 * q**2 + 16 * q + 22
...
def star_score(m: int) -> int:
    """Return the retained exact PG49-star score."""
    q = (4 * m + 3) // 5
    numerator = (
...
def inversion_gain(m: int) -> int:
    """Check the singleton-reversal delta both locally and in closed form."""
    direct = sum(
        (2 * j - 3 * m + 2) * (16 * m + 9 + 2 * j) for j in range(m, 2 * m - 1)
    )
    closed = m * (m - 1) * (m - 2) // 3
...
def check_row(m: int, *, use_dp: bool) -> tuple[int, int, int]:
    ...
    assert score - star_score(m) == inversion_gain(m)
    assert score - k825(m) == 2 * q**2 + 4 * m * q + 8 * q - 5 * m**2 - 14 * m - 4
    assert score - closing_pg46(m) == -2 * (m - q) * (3 * m + q + 4)
    assert score - preclosing_pg46(m) == -2 * (m - q) * (3 * m + q + 4) - 6 * m
    assert star_score(m) < score < k825(m)
    assert score <= closing_pg46(m)
    assert (score == closing_pg46(m)) == (m == 3)
...
def main() -> None:
    ...
    assert monotone_assignment(3) == (0, 1, 2, 4, 5, 3)
    expected_initial = (
        10907,
        23771,
        44160,
    ...
    assert Fraction(286, 10**3) - Fraction(857, 3000) == Fraction(1, 3000)
    assert (
        star_score(3),
        score_formula(3),
        closing_pg46(3),
        k825(3),
        preclosing_pg46(3),
    ) == (10905, 10907, 10907, 10920, 10925)

    print("monotone threshold-closing PG46 exact-K diagnostic: PASS")
    ...
    print("singleton inversion delta and all four comparisons verified")
