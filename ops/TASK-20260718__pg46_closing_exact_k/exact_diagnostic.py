"""Independent exact checks for the closing PG46 core-order K theorem.

This dossier-local script uses only the Python standard library.  It
reconstructs the prescribed PG46 order directly, scores the claimed subset,
checks every oriented shortcut arc, and runs a max-plus dynamic program over
increasing paths.  The DP does not enumerate subsets, permutations, or
matchings.  The all-parameter theorem remains the symbolic proof in
...
def closing_pg46_order(m: int) -> tuple[int, ...]:
    """Expand the PG46 shift that places P_m in the closing gap."""
    assert m >= 3
    n = 10 * m + 3
    d = 8 * m + 4
    v = 2 * m
    alpha = (*range(m), *range(m + 1, v), m)
...
def pg46_formula(m: int) -> int:
    """Exact symbolic score of the closing PG46 order."""
...
def main() -> None:
    ...
    assert closing_pg46_order(3) == expected_m3_order

    dp_transitions = 0
    shortcut_arcs = 0
    observed_initial_scores: list[int] = []
    for m in range(3, MAX_DP_M + 1):
        order = closing_pg46_order(m)
        selected = candidate(m)
        score = cycle_score(order, selected)
        assert score == pg46_formula(m)
        assert score - k825_formula(m) == m**2 - 6 * m - 4
        assert (score < k825_formula(m)) == (m <= 6)
        assert (score > k825_formula(m)) == (m >= 7)
    ...
    assert pg46_formula(3) == 10907
    assert k825_formula(3) == 10920

    print("closing PG46 K diagnostic: PASS")
    ...
    print("minimum row: m=3, K=10907, K825=10920")
    print("pointwise crossover: better m=3..6; worse m>=7; no integer tie")
